...
! Declare namelist parameters
  ...
  ! use_icedms = 
  ! self%use_icedms = use_icedms
#endif

!#if 0
!self%use_icedms = .false.
...
! Register diagnostic variables
   ...
   ! Register environmental variables
    ...
    ! if(any(models.eq.'uvic_icedms'))then  !jpnote 
      if(self%use_icedms) then 
       ...
       call self%register_dependency(self%id_fspdmel,'uvic_icedms_fspdmel','','')  ! but will work if I comment an earlier one -- seems as if there is a limit to space and any more bleeds into space that it shouldnt .. 
       call self%register_dependency(self%id_fspdpon,'uvic_icedms_fspdpon','','')   !memory error   !maybe mismatch of dimensions??  -- not allocating enough space? issue with gfortran complier maybee? try updating compiler?
       call self%register_dependency(self%id_fdmsmel,'uvic_icedms_fdmsmel','','')   !memory error 
       call self%register_dependency(self%id_fdmspon,'uvic_icedms_fdmspon','','')  !memory error 
...
!    _SET_SURFACE_EXCHANGE_(self%id_dms,913./density*(icedms-dms)*(-fdmsmel+fdmspon)/icedms/self%spd*self%zia-fdms3ia/self%spd*self%zia)
    ...
    !hayashida 
    _SET_SURFACE_EXCHANGE_(self%id_dmspd,913./density*botgrowth*(dmspd)+(1000./density*icespd-dmspd)*(-913./density*fspdmel+1000./density*fspdpon)/icespd/self%spd*self%zia-fdmspd3ia/self%spd*self%zia)  !probably this (but double check and be consistent )
